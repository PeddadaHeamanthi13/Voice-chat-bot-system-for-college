...
noise_test=["is","a","this","an","the","are","?","for","of","who","in","many","they",
            ...
            "it","have","if","on","or","st","nd","/","and","has","can","you","was"]

def remove_noise(input_text):
    input_text1=input_text.replace("?","")
    print("input Text is  ",input_text1)
    words=input_text1.split()
    noise_free_words=[word for word in words if word not in noise_test]
    noise_free_text=" ".join(noise_free_words)
    return noise_free_text

app = Flask(__name__)
db= mysql.connector.connect(user='root', database='chatbot')
